...
export default function daterangepicker() {
  ...
  function link(scope, element, attr, daterangeCtrl) {
    $script('/libs/dateRangePicker.min.js', 'dateRangePicker');
    $script.ready('dateRangePicker', function () {
      _load(scope, element, attr, daterangeCtrl);
    });
  }

  function _load(scope, element, attr, daterangeCtrl) {
    ...
    function _setEvents() {
      ...
      events.forEach(function (eventName) {
        ...
        if (angular.isFunction(scope[localEventName])) {
          elem.on(eventName + '.daterangepicker', e => {
            return scope[localEventName]({
              event: e,
              ele: elem
            });
          });
